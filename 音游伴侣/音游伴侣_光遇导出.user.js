// ==UserScript==
// @name         音游伴侣 - 光遇琴谱导出与已下载管理
// @namespace    https://mgm.jie-you.cn/
// @version      1.1.0
// @description  自动捕获网页端解密乐谱，识别已下载状态，一键导出标准光遇(Sky)弹琴JSON并自动维护 downloaded_ids.json
...
(function () {
    ...
    // ========================================================
    // 2. 数据转换：将音游伴侣乐谱转换为光遇 15 键弹琴格式
    // ========================================================
    function convertToSkyFormat(score) {
        ...
        for (const t of tracks) {
            ...
            for (const n of notes) {
                const timeMs = n.startMs ?? n.time ?? n.timeMs ?? 0;
                let keyIdx = -1;

                if (n.rawKey && typeof n.rawKey === 'string' && n.rawKey.includes('Key')) {
                    const m = n.rawKey.match(/Key(\d+)/i);
                    if (m) keyIdx = parseInt(m[1], 10);
                } else if (n.keyIndex != null) {
                    keyIdx = n.keyIndex - 1;
        ...
        songNotes.sort((a, b) => a.time - b.time || a.keyNum - b.keyNum);
        ...
        const skyStudioObj = [
            {
                ...
                bitsPerPage: 16,
                pitchLevel: 0,
                songNotes: songNotes.map(n => ({ time: n.time, key: n.key }))
            }
        ];
        ...
        return {
            ...
            jsonStr: JSON.stringify(skyStudioObj, null, 2),
            rawObject: skyStudioObj
        };
    }
    ...
    function initUI() {
        ...
        panel.innerHTML = `
            ...
            <div class="sky-export-card">
                ...
                <div class="sky-card-header">
                    ...
                    <span class="sky-server-badge" id="sky-server-status" title="运行 python local_sync.py 可实现静默直存本地 scores 目录并更新 JSON">本地服务: 检测中</span>
                </div>
                ...
                <div class="sky-btn-group">
                    ...
                    <button class="sky-btn sky-btn-dl" id="sky-btn-dl" disabled>⬇️ 下载并维护</button>
                </div>
        ...
        document.getElementById('sky-btn-copy').onclick = () => {
            ...
            const { jsonStr } = convertToSkyFormat(capturedScore);
            if (typeof GM_setClipboard !== 'undefined') {
                GM_setClipboard(jsonStr);
            } else {
                navigator.clipboard.writeText(jsonStr);
            }
            showToast('已复制光遇 JSON！可直接粘贴至弹琴脚本');
        };

        document.getElementById('sky-btn-dl').onclick = async () => {
            if (!capturedScore) return;
            const sid = getCurrentScoreId();
            const { title, jsonStr, rawObject } = convertToSkyFormat(capturedScore);
            ...
            if (localServerConnected) {
                try {
                    ...
                    if (ret.success) {
                        ...
                        showToast(`已自动保存到 scores 目录并更新 index！`);
                        return;
                    }
                } catch (e) {
    ...
    function onScoreReady() {
        ...
        const info = convertToSkyFormat(capturedScore);
        dot.classList.add('ready');
        sub.innerText = `已就绪 (${info.totalNotes} 个音符)`;
        btnCopy.disabled = false;
        btnDl.disabled = false;

        const sid = getCurrentScoreId();
        if (isScoreDownloaded(sid)) {
            btnDl.innerText = '⬇️ 重新下载覆盖';
        } else {
            btnDl.innerText = '⬇️ 下载并维护';
        }
    }
    ...
    function startup() {
        initUI();
        checkLocalServer();
        setInterval(markScoreCardsOnList, 1500); // 监听动态加载的乐谱卡片列表
    }
