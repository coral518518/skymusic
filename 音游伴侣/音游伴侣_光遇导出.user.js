...
(function () {
    ...
    // 使用 GM_xmlhttpRequest 彻底解决浏览器从 HTTPS 页面向本地 HTTP 跨域/Mixed Content 报错问题
    ...
    // 与本地同步服务 (local_sync.py:18088) 握手
    async function checkLocalServer() {
        try {
            const status = await gmRequest({
                url: 'http://127.0.0.1:18088/status',
                method: 'GET'
            });
            if (status && status.running) {
                ...
                console.log('[SkyMusic] 🔌 成功连通本地同步服务 (127.0.0.1:18088)，已加载', indexData.total_downloaded, '首记录');
                updateStatusUI();
    ...
    function initUI() {
        ...
        panel.innerHTML = `
            ...
            <div class="sky-export-card">
                ...
                <div class="sky-card-header">
                    ...
                    <span class="sky-server-badge" id="sky-server-status" title="运行 python local_sync.py 可直接写入本地 scores 目录与 downloaded_ids.json">本地服务: 检测中</span>
                </div>
        ...
        document.getElementById('sky-btn-dl').onclick = async () => {
            ...
            if (localServerConnected) {
                try {
                    const ret = await gmRequest({
                        url: 'http://127.0.0.1:18088/save',
                        method: 'POST',
                    ...
                    if (ret && ret.success) {
                        ...
                        showToast(`✅ 已存入 scores 目录并更新 index！`);
                        return;
                    }
                } catch (e) {
    ...
    function updateStatusUI() {
        ...
        if (serverStatus) {
            if (localServerConnected) {
                ...
                serverStatus.innerText = '本地直连: 未连接 ⚠️';
    ...
    function startup() {
        initUI();
        checkLocalServer();
        // 每 3 秒巡检一次本地服务连接与列表标记
        setInterval(() => {
            if (!localServerConnected) checkLocalServer();
            markScoreCardsOnList();
        }, 3000);
    }
