...
def inspect_chaoxi():
    with open("e:/work/skymusic/midi_downloads/潮汐（DJ版）_259271.mid", 'rb') as f: data = f.read()
        ...
        while pos < len(track_bytes):
            ...
            if status == 0xFF:
                ...
                if msg == 0x90:
                    ...
                    if vel > 0 and ch != 9: notes.append((cur_tick, note, vel))
                elif msg in (0x80, 0xA0, 0xB0, 0xE0): pos += 2
                elif msg in (0xC0, 0xD0): pos += 1

    print("潮汐 total notes:", len(notes))
    # Count pitch classes
    pcs = [n[1] % 12 for n in notes]
    names = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
    hist = {names[i]: pcs.count(i) for i in range(12)}
    print("Histogram:", {k: v for k, v in hist.items() if v > 0})

    # Test each shift
    DIATONIC = {0, 2, 4, 5, 7, 9, 11}
    for s in range(-6, 7):
        hits = sum(1 for p in pcs if ((p + s) % 12) in DIATONIC)
        print(f"Shift {s:+2d}: hits={hits}/{len(notes)} ({hits/len(notes)*100:.1f}%)")
